...
class AttentionModule(nn.Module):
    ...
    def forward(self, hidden_states, current_state):
        current_state = current_state.unsqueeze(0).expand(hidden_states.size(0), -1, -1)
        combined = torch.cat([hidden_states, current_state], dim=-1)
        attention_scores = self.attention(combined).squeeze(-1)
        attention_weights = F.softmax(attention_scores, dim=0)
        attended = torch.sum(hidden_states * attention_weights.unsqueeze(-1), dim=0)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = self.query(x).view(-1, batch_size, self.num_heads, self.head_dim)
        k = self.key(x).view(-1, batch_size, self.num_heads, self.head_dim)
        v = self.value(x).view(-1, batch_size, self.num_heads, self.head_dim)
        
        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim)
        attn = F.softmax(scores, dim=-1)
        
        out = torch.matmul(attn, v)
        out = out.view(-1, batch_size, self.num_heads * self.head_dim)
...
class REPLAY(nn.Module):
    ...
    def forward(self, x, t, t_slot, s, y_t, y_t_slot, y_s, h, active_user):
        ...
        # Apply layer normalization before RNN
        poi_time = self.layer_norm1(poi_time)
        ...
        # Compute flashback weights with attention
        out_w = torch.zeros(seq_len, user_len, self.hidden_size, device=x.device)
        for i in range(seq_len):
            # Get attention-weighted historical states
            attended_states, _ = self.attention(out[:i+1], out[i])
            out_w[i] = attended_states
            
        # Apply layer normalization
        out_w = self.layer_norm2(out_w)
